...
const shouldPlayTransition = ref(false)

const isMainPage =
  props.currentPath === '/' ||
  props.currentPath === '/fr/' ||
  props.currentPath === '/en/'
...
const buildGrid = () => {
  ...
  for (let col = COLUMNS; col > 0; col--) {
    ...
    indexes.forEach((randomIndex) => {
      ...
      block.dataset.outDelay = String(
        COLUMNS - col + randomIndex
      )
    ...
    gsap.to(blocks, {
      keyframes: [
        { opacity: 1, duration: 0.01 },
        {
          opacity: 1,
          duration: (_, target) =>
            Number(target.dataset.outDelay) * 0.005
        },
      ...
      stagger: {
        ...
        from: 'start',
        order: (_, target) =>
          Number(target.dataset.outDelay)
      },
      onComplete: () => {
        ...
        shouldPlayTransition.value = false
...
onMounted(async () => {
  ...
  const hasTransitioned =
    sessionStorage.getItem('hasTransitioned') === 'true'
  ...
  shouldPlayTransition.value = true
